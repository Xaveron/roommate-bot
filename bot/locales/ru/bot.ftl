...
help-group =
    ...
    /app — приложение: таблицы, баланс и графики
...
err-turn-changed = Очередь только что изменилась — проверь ещё раз 🙂
...
btn-webapp = 📱 Приложение
webapp-open = 📱 Очередь, история, баланс и статистика «{ $room }» — в приложении:
webapp-open-private = 📱 Приложение открывается в личке — нажми кнопку ниже.
btn-webapp-private = 💬 Открыть в личке
webapp-open-join = 📱 Ты ещё не отметился в «{ $room }». Открой приложение — там можно вступить в комнату.
webapp-not-configured = Приложение пока не подключено.
app-away-set = 🏖 Готово: пропускаю тебя в очередях до { $date } включительно.
app-category-added = ✅ Категория { $title } добавлена! Напоминание в { $time }, каждый день.
app-left = 👋 Ты больше не живёшь в «{ $room }». История сохранится.
app-member-removed = 🚪 { $name } больше не в комнате. Очереди обновлены.
app-export-sent = 📦 Отправил CSV-файлы тебе в личку.
err-dm-needed = Не могу написать тебе в личку — открой бота, нажми «Start» и попробуй ещё раз.
